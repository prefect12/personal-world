...
$('#home-enter').onclick=async()=>{if(busy)return;busy=true;$('#home-enter').disabled=true;$('#transition').classList.add('active');const target=selected;try{
 const worldModule=target==='journey'?null:await import('./worlds.js');await delay(matchMedia('(prefers-reduced-motion: reduce)').matches?0:450);
 ...
 else{view='game';document.body.dataset.view='game';$('#game').hidden=false;try{game=worldModule.createGame({theme:target,onVisit:openPage,visited});}catch(e){console.error('World initialization failed',e);$('#game-fallback').hidden=false;}}
