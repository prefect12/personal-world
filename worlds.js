...
export function createGame({theme,onVisit,visited}){
 ...
 renderer.setAnimationLoop(now=>{
  ...
  boat.position.set(state.x,.13+Math.sin(elapsed*2.1)*.065,state.z);boat.rotation.set(Math.sin(elapsed*1.8)*.025,state.angle,Math.sin(elapsed*1.6)*.038);ship.position.set(state.x,1+Math.sin(elapsed*1.6)*.12,state.z);ship.rotation.y=state.angle;engine.scale.y=(moving?1.8:1)+Math.sin(elapsed*20)*.12;
